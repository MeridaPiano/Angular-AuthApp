...
export class AuthService {
  ...
  async create(createUserDto: CreateUserDto): Promise<User> {
    ...
    try {
      ...
      if( error.code === 11000 ) {
        throw new BadRequestException( `${ createUserDto.email } alreadey exists!`);
      }
  ...
  async login( loginDto: LoginDTO): Promise<LoginResponse>{
    ...
    return { 
      user: rest, 
      token: this.getAuthToken({id: user.id})
    };
  };
  ...
  async register( registerUserDto: RegisterDto ): Promise<LoginResponse>{
    ...
    try{
      ...
      return {
        user: user,
        token: this.getAuthToken({ id: user._id })
      };
  ...
  getAuthToken(payload: JwtPayload){
